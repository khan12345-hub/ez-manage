...
export function useBoardFilters({ columns }: Props) {
  ...
  const hideableColumns = useMemo(() => {
    return columns.filter((column: any) => !column.isPrimary);
  }, [columns]);
  ...
  const toggleAllColumns = (checked: boolean) => {
    ...
    setHiddenColumnIds(hideableColumns.map((column: any) => column.id));
  };

  const filteredColumns = useMemo(() => {
    return columns.filter(
      (column: any) => !hiddenColumnIds.includes(column.id),
    );
  }, [columns, hiddenColumnIds]);

  const allColumnsHidden =
    hideableColumns.length > 0 &&
    hideableColumns.every((column: any) =>
  ...
  const selectedPersonIds = useMemo(
    () => personFilter?.users.map((user) => user.id) ?? [],
    [personFilter],
  );
